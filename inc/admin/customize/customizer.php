 ...
 * @since 1.0.0
 * @param WP_Customize_Manager $wp_customize WP's instance of customizer.
 */
function greenlight_customize_register( $wp_customize ) {
	...
	if ( $types = greenlight_get_color_types() ) {
		...
		foreach ( $types as $key => $args ) {
			...
			if ( ! empty( $args['type'] ) && 'color' === $args['type'] ) {
...
function greenlight_customize_scripts() {
	...
	wp_enqueue_style(
		'greenlight-customize-controls',
		esc_url( get_template_directory_uri() . '/assets/css/customize-controls.css' ),
		array(),
		GREENLIGHT_VERSION
	);
...
function greenlight_get_top_bar_items() {
	...
	return apply_filters( 'greenlight_top_bar_items', array(
	    'top_text' => array(
	        ...
	        'description'       => sprintf( esc_html__( 'Use any %1$s icon like %2$s.', 'greenlight' ), '<a href="http://fontawesome.io/icons/" target="_blank">FontAwesome</a>', '<code>%%icon_name%%</code>' ),
